...
> **標準前提**: ラポルタの概算見積・提案は、価格最優先の最低仕様ではなく
> 「下地処理を省かず、仕上がりと持ちを優先する丁寧プラン」を基本線にする。

- 既存不陸、浮き、クラック、剥離、含水、錆、汚れは「一定の補修・調整が必要」と仮定して概算を組む
- 塗装はケレン・清掃・養生・必要箇所のパテ/下地調整を先に見込む
- クロスは下地補修込み、床はレベリングや不陸調整の可能性を初期段階で注記する
- 鉄部はケレン + 錆止めを省略しない前提で説明する
- 現調後に「状態が良く補修を減らせる」場合は減額方向もあり得ると伝える

### 丁寧プラン確認の追加質問

| 項目 | 型 | 例 |
|------|----|----|
| 11 | 優先したいこと | セレクト | とにかく安く/バランス重視/仕上がりと耐久性重視 |
| 12 | 既存状態で気になる点 | チェック + 自由記述 | ひび/剥がれ/凹凸/錆/カビ/不明 |

### 夜間対応・日数分割の追加質問

| 項目 | 型 | 例 |
|------|----|----|
| 13 | 作業可能時間帯 | セレクト | 日中のみ/夜間のみ/どちらでも |
| 14 | 夜間制約の有無 | チェック + 自由記述 | 音出しNG/共用部搬入は21時以降/朝までに原状復帰 |
| 15 | 分割施工の希望 | セレクト | 一括短期/営業しながら日数分割/休業日に集中 |

> **運用ルール**: 店舗・テナント案件で「営業を止めたくない」「夜しか触れない」が出たら、
> 初回概算から夜間対応と日数分割の可能性を前提に拾う。
...
### 概算見積の既定モード: 下地処理丁寧プラン

問い合わせ段階の初回概算は、以下を標準で見込む。

- 内装壁天井: パテ、不陸調整、既存下地補修を含む前提
- 床: レベリングや部分補修の余地を見込む前提
- 塗装: ケレン、清掃、養生、必要な下塗りを含む前提
- 鉄部: 3種ケレン相当 + エポキシ系錆止め下塗りを基本前提
- 外装: 足場別途、下地処理を省かない前提

> **営業ルール**: 安さを作るために下地工程を抜いた概算は初回提示の標準にしない。
> 価格勝負ではなく、施工後のやり直しや追加請求リスクを減らす提案として説明する。

### 施主への説明テンプレ（概算提示時）

```
今回の概算は、仕上げだけを薄く見るのではなく、
下地処理を丁寧に行う前提で組んでいます。

理由は、見た目だけ先に整えても、下地の傷みや不陸が残ると
後から剥がれ・割れ・ムラとして出やすいためです。

現地確認の結果、下地状態が良ければ減額できる可能性もありますが、
最初は「あとで困らない仕様」を基準にご案内します。
```
...
### 概算見積の追加モード: 夜間対応・日数分割

夜間対応や営業継続しながらの分割施工が必要な案件では、初回概算で以下を上乗せ前提にする。

- 夜間人工: 職人1人工あたり `¥45,000` を下限基準に置き、通常日当への率掛けでは済ませない
- 諸経費: 夜間管理、搬入出調整、警備・立会い、近隣配慮を別枠で加算
- 搬入出・養生復旧の繰り返し: 日数分割回数に応じて加算
- 騒音制約による手待ち: 解体・斫り・切断などは別日化または短時間化
- 毎回の原状復帰: 通路確保、清掃、翌営業に向けた仮復旧を加算
- 1日ズレ吸収バッファ: 夜間案件は工程ずれ1日分で赤字化しやすいため、最低でも `1人工 + 諸経費` の安全側バッファを概算へ含める
- 利益基準: `1日ずれたら赤字` の見積は初回提示しない。粗利は夜間・分割リスクを吸収できる水準まで先に載せる

#### 夜間対応時の営業ルール

- 「夜だから早い」ではなく「夜だから回数と養生復旧が増える」前提で説明する
- 金額だけでなく、昼工事より日数が伸びやすい点を初回から伝える
- 施主が営業継続を優先する場合は、総額より営業損失回避の価値で比較する
- `併売で吸収` を前提に値付けしない。単体案件で原価超過しないことを優先する

#### 施主への説明テンプレ（夜間・分割施工）

```
夜間対応や営業しながらの分割施工も可能です。

ただしその場合は、毎回の養生・片付け・翌営業に向けた復旧、
作業時間制限による工程分割に加えて、夜間人工と管理経費が増えるため、
日中一括施工より日数と費用が上がりやすいです。

今回の概算では、夜間人工・諸経費・工程ずれバッファまで含めた
安全側の金額で組んでいます。
現地条件が整理できれば、どこをまとめて短縮できるかもご提案します。
```
...
| ケース | 工期の見方 | 追加前提 |
|--------|------------|----------|
| 夜間のみ施工 | 日中標準工期 × 1.2〜1.5 | 夜間人工 `¥45,000/人工` 下限、諸経費別途、静音作業への組替 |
| 営業しながら日数分割 | 日中標準工期 × 1.3〜1.8 | 毎回の養生復旧、区画分け、朝までの仮復旧、1日ズレ吸収バッファ |
| 休業日に集中施工 | 日中標準工期 × 1.1〜1.3 | 短期集中の増員、時間外対応 |

#### 工程表の出し分けルール

- 施主が営業継続優先なら「総日数」と「1回あたりの触れる範囲」をセットで出す
- 夜間案件は `作業時間` `音出し可能時間` `翌営業までの復旧締切` を工程表に持たせる
- LINE報告は `何日目` だけでなく `第何夜` `次回でどの区画まで終わるか` も添える
